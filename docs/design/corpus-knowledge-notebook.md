...
| **3 · OBLIGATION** | what is *owed*: heirs, indictments, ratified-but-unbuilt, negative decisions, expiring premises, acceptance criteria, enumeration completeness | **ABSENT** | 651 `heir`s · 29 stub heirs (one now **void**, § 3.6c) · *"what this indicts today"* → `DeepAxis` still present · *"ratified, never built"* → RATE **built 2026-07-29 (journal/0123)**, by a slice rather than by a watcher · #35: *"a decision not to build gets a paragraph in a doc comment and **no watcher**"* · T6: no enumeration is ever checked for completeness |
...
| **ratified-but-unbuilt** | `### RATE — ratified 2026-07-24, never built` | RATE, ~~**still unbuilt**~~ **BUILT 2026-07-29 (journal/0123)** — the exemplar was retired by a slice, not by a watcher |
