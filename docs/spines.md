...
the same day (journal/0128–0129), its ~460 m RECORD half NOT STARTED.** *The
...
one. What landed near: the member dither's source (a single bilinear octave at
...
of it. What did not: the per-column choice among the touched cells' records, whose
blast radius measured 13 files / ~40 sites because `ColumnRec.strata` is a `pub`
field.* A boundary type at the
