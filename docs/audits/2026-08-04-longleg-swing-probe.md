...
| **3. per-cycle quantization** (journal/0155) | **RULED OUT as the cause; it is an AMPLIFIER** | the discontinuity is fully present in the **continuous** trace, which never consults the pose grid: **5.0200°**, six crossings, at a phase that also does not move with *speed* (§ 5) — the property an aliasing artifact could not have. What quantization does is *enlarge* the visible step, because the held pose lands further down the steep stretch: 5.0200° continuous → **12.2558°** at `--anim-fps 60` (N = 63) → **19.2112°** at `--anim-fps 12` (N = 13). It also decides *how many* of the six survive: 4 at 60 fps, 2 at 12 fps. |
