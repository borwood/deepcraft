...
| **P11** | **MEMBERS INTO DEEP HISTORY** — the deep record goes member-grade; identity is recorded, expression expresses | **DECIDED 2026-08-01 (user), TOP PRIORITY** — ROADMAP § Sequenced "MEMBERS INTO DEEP HISTORY" (the arc anchor; user verbatim there) + `materials.md` § DECIDED 2026-08-01. First slice = **design pass** (representation · residency envelope · fate of the Litho roster · migration · THE RIPPLE MAP). **Nothing slated proceeds against the class-grade record without checking the ripple map**: record-terms slice, ruling 3 (HELD), P10, MM-3/near-path restructure, member #1, P5. Spans the partition on purpose — the *record* is E2 storage, the *identities* are pack content; the design pass draws that line |
