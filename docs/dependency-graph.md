...
| **E4** | **field-solver primitives** — the S-10 gather; **the kernel owns its own stability bound** | **SHAPE NAMED 2026-07-29 (S-10), 2 instances, NOT EXTRACTED. DESIGN PASS DISPATCHED 2026-08-03** (user: in-band gen time "actually unacceptable" — the P2 ladder measured sub-steps ∝ M, 2 → 896, pure explicit-stability tax; the implicit unconditionally-stable kernel is the ruled answer-shape for P2 finding 7, Braun & Willett lineage). Also the named fix for the deferred pit-safari's 38-min world cost | every future diffusing pass · P2's flip (gen-time conflict) · cheap high-M worlds |
