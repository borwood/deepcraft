...
**Maintained in-session** — see § Process at the bottom. Last full pass: **2026-07-29**; rows reconciled at the **2026-08-04 GEO-4 wrap** (E4, E5/P11 correlation, PARENT MATERIALS).
...
| **E4** | **field-solver primitives** — the S-10 gather; **the kernel owns its own stability bound** | **E4-1 EXTRACTED 2026-08-03** (byte-identically, all goldens green): `dc-core::field::FieldKernel` — explicit scheme, flux-form, the bound + sub-cycle derivation kernel-owned, the pass declares only its coefficient field; **✅ VENUE RATIFIED 2026-08-04 (U-3, user: "The engine owns primitives… we're not going to entrench a primitive in worldgen, whose heir is the pack"; rayon rider included; U-5 closed as executed)**. Creep declares against it; `sat.rs` NOT converted (E4-1b — needs `PairMin` + upper obstacle, the audit's E4-2b shape-proof). Design pass 2026-08-03 (`docs/audits/2026-08-03-e4-implicit-kernel-design.md`; user: in-band gen time "actually unacceptable" — the P2 ladder measured sub-steps ∝ M, 2 → 896, pure explicit-stability tax; the implicit unconditionally-stable kernel is the ruled answer-shape for P2 finding 7, Braun & Willett lineage). **✅ ALL FIVE PICKS RULED 2026-08-04** (audit header): U-1 **K2** (BE-factorized ADI + fixed-k Picard + today's limiter as final projection; K1 rejected, K3 deferred, **no fallback pre-committed** — K2-alt vs K3 re-priced on numbers if the study demands escalation) · U-2 **always-implicit at E4-3** ("no dual-worlds from one seed") · U-3 **dc-core** · U-4 **the bar is INSTRUMENT-MEANING CONTINUITY, not world fidelity** (derived from ladder rung-noise; beyond it the ladder re-runs and prior numbers become integrator-scoped; only the structural fixtures — anisotropy, cascade — are hard rejections) · U-5 executed. **E4-2 BUILT BUT NOT ADOPTABLE AS RULED — measured 2026-08-04** (`docs/audits/2026-08-04-e4-2-convergence-study.md` on main; **code unmerged**, `worktree-agent-a787b0d7fdeb61b53`): the limiter chain IS monotone to a=1e8; **Picard relinearization breaks monotonicity at a=2.0**, and the *binding* anisotropy bound is **0.85**, where the arm is **2.2× SLOWER than explicit** at picard=4 — `picard=1` overshoots the max principle by 150 m. **Fork owed: re-price K2-alt vs K3, or park** (U-1 pre-committed to neither). § 5's world runs unrun · **E4-3** (adoption, sequenced against the P2 flip so goldens move once, with the M re-pick). Also the named fix for the deferred pit-safari's 38-min world cost | every future diffusing pass · P2's flip (gen-time conflict) · cheap high-M worlds |
...
| **E6** | **open resource vocabulary** — `DeepAxis` retires; packs declare their own ids | **UNBLOCKED 2026-07-29** (E3 landed); sequenced. ⚠ **SCOPED TOO NARROWLY — 2026-08-05.** `DeepAxis` is one of **at least three** closed engine enums naming pack concepts: **`FlowCause`** (`flux.rs:337-352`, seven transport regimes + `MOVER_NONE` = **exactly full at 3 bits**, and it lives **in every `DepUnit` and in the merge key**) and **`Cause`** (`inventory.rs:395-407`, four weathering agents). `Dissolution` sits in **both**, owned by neither. `ARCHITECTURE.md`'s ratified indictment (DECIDED 2026-07-26) names only `DeepAxis` — **the enumeration is incomplete and completing it is owed**. Replacement shape needs nothing new: a compact id into a pack-declared registry, exactly as `MaterialId` works. **User framing 2026-08-05:** *"they are pack opinions living in the engine. The engine has no concept of earth science processes… we must not foreclose or inject weird opinions into other future mods / plugin packs."* Test: **would a non-Earth pack inherit this?** `Fluvial`/`Frost`/`Glacial` are Earth concepts. ⚠ `DeepAxis` stays entangled with **E7** (authored order removes its *reason* to exist); `FlowCause`/`Cause` are entangled with nothing, and `FlowCause` **converges with the record-schema work** since both touch `DepUnit`'s packed layout | third-party packs · **the mover vocabulary** · a clean weathering-cause split |
| **E9** | **the stratigraphic correlation kernel** — `deeptime::correlate` (`Bed`/`Borehole`/`EpochPartition`/`ColumnInterval`/`bilinear_weights`) | ✅ **EXTRACTION TO `dc-core` RULED 2026-08-05 (user)**, same ruling as E4-1's venue (U-3: *"the engine owns primitives… we're not going to entrench a primitive in worldgen, whose heir is the pack"*). **The subtraction test passes at source — verified 2026-08-05: the module contains NO `MaterialId` and NO `GeologySet`.** Subtract the whole pack and it is untouched. ⬦ **Do it BEFORE S2 lands on it**; every slice built on top enlarges the extraction | S2 · every future refinement operator reading the record |
...
**B9 — THE PACK-OWNED MOVEMENT LAYER (user, 2026-08-05).** Added by the turn delete (journal/0159,
slice one of ROADMAP § *DIRECTION IS AN AXIS, NOT A RATE*). `TRUNK_TURN_WINDOW_S` is gone and the
engine's neutral turn is **instant**; the replacement is a **pack-owned layer that movement intent
passes through** — *"may be opinionated or bare"* — with **both extremes as requirements**: a body
that snaps and pivots instantly, and a body that preserves momentum and must slow to come about.
Bare passthrough (today's behaviour) is one of the two ends, not the degenerate case.
- **Partition:** it is an **OPINION** under § 0b — two well-made packs genuinely disagree about how
  a body comes about — so it is pack content, and the engine owns only the seam it passes through.
- **Site:** `dc-api/src/character.rs::step_character`, where intent becomes velocity, marked
  `stubs.md` **#55**. Deliberately **not** the deleted constant's site: the constraint belongs on
  the **movement**, not the trunk yaw.
- **Blocked on nothing.** Independent of B2/B3 — it consumes intent, not gait. It will eventually
  **feed** relative-axis gait (the arc's second half), because backing and strafing are the same
  axis seen at 180° and 90°.
- **⚠ It also absorbs the PLAYER (user, same day; `stubs.md` #56): *"there will be no separate
  player concept except that the body is driven by user instead of script or mcp."*** Three drivers
  — user, script, MCP — one layer. `dc-client/src/player.rs`, currently cited by the character
  primitive's own module doc as *"the reference for these dynamics"*, is a **stand-in**. Not
  scheduled; recorded at the ruling.
