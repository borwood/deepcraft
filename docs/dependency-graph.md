...
| **P2** | `EROSION_CALIBRATION` re-pick + flag flip | **LITERATURE DERIVATION SHIPPED 2026-08-02** (`ee9fb94`, craton ruling in its header: present-state band governs; bracket `M ∈ [60, 240]`, twice-derived target agreeing to 5 %). **Remaining: the measurement runs** (need the build slot; queued behind P11 slice 2). 45 was fitted to the broken solve and inverts under the fixed one |
