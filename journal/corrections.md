...
## 90. "A provider slot's health is checked by its consumers — if the seam still has readers, the heir can still arrive" (the seam doctrine's unstated converse — nearly cost P11 slice 2 the `outcrop_shares` seam's actual customer, 2026-08-02; filed by the slice, ordinal at merge)
...
## 91. "The identity path's structures are empty when the tier is off, so the off path cannot touch them" (P11 slice 2's dense→sparse conversion, 2026-08-02 — an arithmetic expression became a lookup with a precondition; caught at the slice's own gate, fixed never captured)
